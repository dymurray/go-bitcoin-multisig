...
func TestGenerateFund(t *testing.T) {
	btcutils.SetFixedNonce = true //SetFixedNonce set to true to get repeatable signatures with a fixed nonce for testing.
	{
		...
		finalTransactionHex := generateFund(testPrivateKeyWIF, 0, testInputTx, testAmount, testP2SHDestination)
...
func TestSignP2PKHTransaction(t *testing.T) {
	{
		...
		signedTx, err := signP2PKHTransaction(testRawTx, 0, testPrivateKey, testScriptPubKey, testInputTx, testAmount)
		if err != nil {
			t.Error(err)
		}
